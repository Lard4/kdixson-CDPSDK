...
BloomFilter::BloomFilter(int numSectors, int numHashes, int bitsPerSector, int maxMsgs) {
   ...
   logdebug("initialize bloom filter 1");
   // Initialize the bloom filters, fill with 0's
   this->filter1 = new unsigned int[this->numSectors];
   logdebug("Filter 1 address %p\n", this->filter1);
   if (this->filter1 == NULL) {
      logdebug("Memory allocation for Bloom Filter 1 failed!\n");
      exit(0);
   }
   for (int n = 0; n < this->numSectors; n++) {
      this->filter1[n] = 0;
   }
   logdebug("Initialized BF1, %d slots, %d this->numSectors\n", numSectors, this->numSectors);

   logdebug("initialize bloom filter 2");
   this->filter2 = new unsigned int[this->numSectors];
   logdebug("Filter 2 address %p\n", this->filter2);
   if (this->filter2 == NULL) {
      logdebug("Memory allocation for Bloom Filter 2 failed!\n");
      exit(0);
   }
   for (int n = 0; n < this->numSectors; n++) {
      this->filter2[n] = 0;
   }
   logdebug("Initialized BF2, %d slots, %d this->numSectors\n", numSectors, this->numSectors);

   logdebug("initialize random seeds");
   //get random seeds for hash functions
   int* Seeds = new int[numHashes];
   if (Seeds == NULL) {
      logdebug("Memory allocation for seeds failed!\n");
      exit(0);
   }
   srand(time(NULL));
   int i;
   ...
   for (i = 0; i < numHashes; i++){
      logdebug("random seed %d: %d\n", i+1, Seeds[i]);
   }
   this->Seeds = Seeds;

   logdebug("bloom_init end");
}
...
void BloomFilter::bloom_add(unsigned char* msg, int msgSize) {
   ...
   if (this->nMsg >= this->maxMsgs) {
      if (this->activeFilter == 1){
         logdebug("Freezing filter 1, switching to filter 2\n");
         ...
         logdebug("Freezing filter 2, switching to filter 1\n");
         for (int i = 0; i < (this->numSectors)/(this->bitsPerSector); i++) {
            this->filter1[i] = 0;
         }
         this->activeFilter = 1;
